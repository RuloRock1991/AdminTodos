...
export const TabBar = ( {tabOptions = [1, 2, 3, 4], currentTab = 1}: Props) => {
    ...
    const tabs = tabOptions.length;

    const [selected, setSelected] = useState(currentTab);

    const onTabSelected = (tab:number) => {
        setSelected(tab);
        setCookie("selectedTab", selected);
        router.refresh();
    };

    return (
        <div className={`grid w-full space-x-2 rounded-xl bg-gray-200 p-2 grid-cols-${tabs}`}>
            {
                tabOptions.map(tab => (<div key={tab}>
